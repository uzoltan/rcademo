...
public class SendAndReceive {
  ...
  protected static class MessageCallback implements com.microsoft.azure.sdk.iot.device.MessageCallback {
    ...
    public IotHubMessageResult execute(Message msg, Object context) {
      ...
      System.out
          .println("Received message " + counter.toString() + " with content: " + new String(msg.getBytes(), Message.DEFAULT_IOTHUB_MESSAGE_CHARSET));
  ...
  public static void main(String[] args) throws IOException, URISyntaxException {
    ...
    if (args.length <= 1 || args.length >= 5) {
      System.out.format("Expected 2 or 3 arguments but received: %d.\n" + "The program should be called with the following args: \n"
                            + "1. [Device connection string] - String containing Hostname, Device Id & Device Key in one of the following formats: "
    ...
    if (args.length == 2) {
      ...
      if (protocolStr.equals("https")) {
        ...
        System.out.format("Expected argument 2 to be one of 'mqtt', 'https', 'amqps' or 'amqps_ws' but received %s\n"
                              + "The program should be called with the following args: \n"
                              + "1. [Device connection string] - String containing Hostname, Device Id & Device Key in one of the following "
                              + "formats: HostName=<iothub_host_name>;DeviceId=<device_id>;SharedAccessKey=<device_key>\n"
                              + "2. [number of requests to send]\n" + "3. (mqtt | https | amqps | amqps_ws | mqtt_ws)\n"
                              + "4. (optional) path to certificate to enable one-way authentication over ssl for amqps \n", protocolStr);
        return;
      }
